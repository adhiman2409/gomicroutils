...
const EMPLOYEE_COLLECTION_GEO_TRACKING_DAILY_REPORT = "employee-geo-tracking-daily-report"
...
const EMPLOYEE_COLLECTION_MANUAL_LEAVES = "employee-manual-leaves"
...
const JOBMGR_COLLECTION_RECRUITERS = "jobmgr-recruiters"
const JOBMGR_COLLECTION_APPLICATION_COUNTER = "jobmgr-application-counter"
const JOBMGR_COLLECTION_APPLICATION_STATUS = "jobmgr-application-status"
...
const PAYMENTS_COLLECTION_TENANT_ADDRESS_COLLECTION = "payments-tenant-address-collection"
const PAYMENTS_BANK_DETAILS_COLLECTION = "payments-bank-details-collection"
...
func GetStickyNoteCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(KYC_COLLECTION_STICKY_NOTE)
}

func GetPaymentsBankDetailsCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(PAYMENTS_BANK_DETAILS_COLLECTION)
}
...
func GetPayrollSalaryProcessingConfigCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(PAYROLL_COLLECTION_SALARY_PROCESSING_CONFIG)
}
...
func GetEmpGeoTrackingDailyReportCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(EMPLOYEE_COLLECTION_GEO_TRACKING_DAILY_REPORT)
}
...
func GetEmpManualLeavesCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(EMPLOYEE_COLLECTION_MANUAL_LEAVES)
}
...
func GetJobMgrApplicationStatusCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(JOBMGR_COLLECTION_APPLICATION_STATUS)
}
...
func GetJobMgrRecruitersCollection(client *mongo.Client, domain string) *mongo.Collection {
	db := client.Database(strings.Replace(domain, ".", "_", -1))
	return db.Collection(JOBMGR_COLLECTION_RECRUITERS)
}
